...
@concrete struct BEMTRotorCACompSideFlow <: OpenMDAOCore.AbstractImplicitComp
...
function OpenMDAOCore.setup(self::BEMTRotorCACompSideFlow)
    ...
    push!(input_data, OpenMDAOCore.VarData("v", shape=num_operating_points, val=1., units="m/s"))   # normal to disk
    push!(input_data, OpenMDAOCore.VarData("v_pal", shape=num_operating_points, val=0., units="m/s"))   # parallel to disk
    ...
    push!(output_data, OpenMDAOCore.VarData("phi", shape=(num_operating_points, num_radial, num_azimuth), val=1.0, units="rad"))   # [i, j, l]
    push!(output_data, OpenMDAOCore.VarData("thrust", shape=num_operating_points, val=1.0, units="N"))
    push!(output_data, OpenMDAOCore.VarData("drag", shape=num_operating_points, val=0.0, units="N"))
    ...
    push!(partials_data, OpenMDAOCore.PartialsData("phi", "v_pal", rows=rows, cols=cols))
    ...
    push!(partials_data, OpenMDAOCore.PartialsData("drag", "Rhub", rows=rows, cols=cols))
    push!(partials_data, OpenMDAOCore.PartialsData("drag", "Rtip", rows=rows, cols=cols))
    ...
    push!(partials_data, OpenMDAOCore.PartialsData("drag", "radii", rows=rows, cols=cols))
    push!(partials_data, OpenMDAOCore.PartialsData("drag", "chord", rows=rows, cols=cols))
    push!(partials_data, OpenMDAOCore.PartialsData("drag", "theta", rows=rows, cols=cols))
    ...
    push!(partials_data, OpenMDAOCore.PartialsData("thrust", "v_pal", rows=rows, cols=cols))
    ...
    push!(partials_data, OpenMDAOCore.PartialsData("drag", "v", rows=rows, cols=cols))
    push!(partials_data, OpenMDAOCore.PartialsData("drag", "v_pal", rows=rows, cols=cols))
    push!(partials_data, OpenMDAOCore.PartialsData("drag", "omega", rows=rows, cols=cols))
    push!(partials_data, OpenMDAOCore.PartialsData("drag", "pitch", rows=rows, cols=cols))
    push!(partials_data, OpenMDAOCore.PartialsData("drag", "drag", rows=rows, cols=cols, val=-1.0))
    push!(partials_data, OpenMDAOCore.PartialsData("torque", "v", rows=rows, cols=cols))
    push!(partials_data, OpenMDAOCore.PartialsData("torque", "v_pal", rows=rows, cols=cols))
    ...
    push!(partials_data, OpenMDAOCore.PartialsData("efficiency", "v_pal", rows=rows, cols=cols))
    ...
    push!(partials_data, OpenMDAOCore.PartialsData("drag", "phi", rows=rows, cols=cols))
...
function OpenMDAOCore.solve_nonlinear!(self::BEMTRotorCACompSideFlow, inputs, outputs)
...
#+ results="hidden"
function OpenMDAOCore.apply_nonlinear!(self::BEMTRotorCACompSideFlow, inputs, outputs, residuals)
    # Unpack all the options.
    num_operating_points = self.num_operating_points
    num_blades = self.num_blades
    num_radial = self.num_radial
...
function OpenMDAOCore.linearize!(self::BEMTRotorCACompSideFlow, inputs, outputs, partials)
...
# OpenMDAOCore.detect_guess_nonlinear(::Type{<:BEMTRotorCACompSideFlow}) = false
# OpenMDAOCore.detect_apply_linear(::Type{<:BEMTRotorCACompSideFlow}) = false
