...
from ccblade_openmdao_examples.ccblade_openmdao_component_nonper import BEMTRotorCAComp
from julia.OpenMDAO import make_component

# similar runscript to run_ca.py, but include non-perpendicular inflow
...
def get_problem():
    ...
    # -----------------------
    # velocity component parallel to the disk
    # -----------------------
    v_pal = 30.0 * np.ones(num_operating_points)
    ...
    comp = make_component(
        BEMTRotorCAComp(
            ...
            num_radial=num_radial, num_azimuth=10, rho=rho0, mu=mu, speedofsound=speedofsound))
    prob.model.add_subsystem("bemt_rotor_comp", comp, promotes_inputs=["Rhub", "Rtip", "radii", "chord", "theta", "v", "v_pal", "omega", "pitch"], promotes_outputs=["thrust", "torque", "efficiency"])
...
if __name__ == "__main__":
    ...
    # om.n2(p)
    # p.run_model()
    # p.check_partials(compact_print=True, abs_err_tol=1e-4, rel_err_tol=1e-4)
